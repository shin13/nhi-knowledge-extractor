...
This file is the primary onboarding document for **both human contributors and AI coding agents** (Claude Code, Copilot, Cursor, etc). New session? Read this first, then dive into `docs/` per the "Project history & lessons learned" section.

Convert Taiwan NHI medication regulation DOCX documents into RAG-ingestion-ready CSV deliverables. Successor to `NHI-Knowledge-Extraction`. See `docs/spec.md` for the full design.
...
CHANGELOG.md                     project version history (Keep-a-Changelog, manual)
data/regulations/medication/CHANGELOG_data.md   per-release NHI data diff (gitignored, pipeline-generated)
```
...
**Commit:**
- Source under `src/`, tests, docs, `pyproject.toml`/`uv.lock`, `CHANGELOG.md` (project history)
- Small fixed `tests/fixtures/*.docx` — these are pinned regression inputs, never change
- `CLAUDE.md` — this file, public agent guide
...
- `data/regulations/medication/CHANGELOG_data.md` — pipeline-generated NHI release log, regenerated by `nhi-extract sync`.
- `.private/` — local-only dev notes (HANDOFF / session state). Not relevant to outside contributors.
...
3. [`docs/emit-depth-plan.md`](docs/emit-depth-plan.md) — design rationale for `EMIT_DEPTH` parameter, RAG metadata (`parent_id`/`part_index`/`total_parts`), and anchor preamble. Read this before touching the chunker.
4. [`docs/roadmap.md`](docs/roadmap.md) — categorised future plans (P0–P6) with YAGNI status. Where to look first when picking up the project.
5. [`docs/next-fixes.md`](docs/next-fixes.md) — historical work queue. Tasks A–H landed; Task G (附表 structured CSV) is the open item.
